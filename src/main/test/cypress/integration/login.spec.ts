...
describe('Login', () => {
  ...
  it('Should load with correct initial state', () => {
    ...
    cy.getByTestId('email')
      .should('have.attr', 'title', 'Campo obligatorio')
      .should('have.attr', 'readOnly')
    cy.getByTestId('email-label').should('have.attr', 'title', 'Campo obligatorio')
    cy.getByTestId('password-wrap').should('have.attr', 'data-status', 'invalid')
    cy.getByTestId('password')
      .should('have.attr', 'title', 'Campo obligatorio')
      .should('have.attr', 'readOnly')
    cy.getByTestId('password-label').should('have.attr', 'title', 'Campo obligatorio')
  ...
  it('Should present invalid credentials on 401', () => {
    ...
    cy.getByTestId('main-error').should('contain.text', 'Credenciales invalidas')
  ...
  it('Should present UnexpectedError on 400', () => {
    ...
    cy.getByTestId('submit').click()
    cy.getByTestId('spinner').should('not.exist')
    cy.getByTestId('main-error').should('contain.text', 'Sucedio algo')
    cy.url().should('eq', `${baseUrl}/login`)
  })

  it('Should present UnexpectedError if invalida data is returned', () => {
    cy.route({
      method: 'POST',
      url: /login/,
      status: 200,
